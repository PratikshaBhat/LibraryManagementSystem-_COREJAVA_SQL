...
public class Return_book extends JFrame implements ActionListener{
    ...
    public void delete() {
        try {
            conn con = new conn();
            String sql = "delete from IssueBook_db where Book_id=?";
    ...
    public void actionPerformed(ActionEvent ae){
        try{
            conn con = new conn();
            if(ae.getSource() == b1){
                String sql = "select * from issueBook_db where Student_id = ? and Book_id =?";
                PreparedStatement st = con.c.prepareStatement(sql);
            ...
            if(ae.getSource() == b2){
                String sql = "insert into returnBook_db(Book_id, Student_id, BName, SName,Course, Branch, DateOfIssue, DateOfReturn) values(?, ?, ?, ?, ?, ?, ?, ?)";
                PreparedStatement st = con.c.prepareStatement(sql);
                st.setString(1, textField.getText());
